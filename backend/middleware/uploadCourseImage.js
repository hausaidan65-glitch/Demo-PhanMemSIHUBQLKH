...
const fileFilter = (req, file, callback) => {
  const allowedMimeTypes = [
    ...
    // Một số công cụ có thể gửi kiểu này
  ...
  console.log("UPLOAD COURSE IMAGE:", {
  ...
  /*
   * application/octet-stream chỉ được chấp nhận
   * nếu phần mở rộng thuộc danh sách ảnh.
   */
  if (!validMimeType || !validExtension) {
    return callback(
      new Error(
        `Ảnh không hợp lệ. File nhận được: ${file.originalname}, loại: ${file.mimetype}. Chỉ hỗ trợ JPG, JPEG, PNG, WEBP hoặc JFIF.`,
      ),
    );
  }
