...
import networkingProgressApi from '../../../services/networkingProgressApi';
import NetworkingProgressHistory from './NetworkingProgressHistory';
...
export default function NetworkingProgressModal({ event, onClose }) {
  ...
  useEffect(() => {
    ...
    networkingProgressApi
      .getNetworkingProgress(event.id)
      .then((response) => {
        if (active) {
  ...
  const handleSubmit = async (submitEvent) => {
    ...
    try {
      const response = await networkingProgressApi.createNetworkingProgress(
        event.id,
  ...
  return (
    <div className="fixed inset-0 z-[170] flex items-center justify-center bg-black/50 p-4">
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="networking-progress-title"
        className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"
      >
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
          <div>
            <h2
              id="networking-progress-title"
              className="text-xl font-bold text-slate-900"
            >
              Cập nhật tiến độ Sự kiện kết nối
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              {event.event_name || `Sự kiện #${event.id}`}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-y-auto px-6 py-5">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label
                htmlFor="networking-progress-content"
                className="mb-2 block text-sm font-medium text-slate-700"
              >
                Nội dung tiến độ <span className="text-red-500">*</span>
              </label>
              <textarea
                id="networking-progress-content"
                rows={4}
                value={content}
            ...
            <div>
              <label
                htmlFor="networking-progress-note"
                className="mb-2 block text-sm font-medium text-slate-700"
              >
                Ghi chú quan trọng
              </label>
              <textarea
                id="networking-progress-note"
                rows={3}
                value={note}
          ...
          <section className="mt-8 border-t border-slate-200 pt-6">
            ...
            {historyLoading ? (
              ...
              <NetworkingProgressHistory history={history} />
            )}
          </section>
        </div>
      </div>
