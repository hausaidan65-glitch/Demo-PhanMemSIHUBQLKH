...
export default function Program() {
  ...
  const validateForm = () => {
    ...
    if (form.has_project) {
      ...
      if (!form.project_description.trim()) {
        nextErrors.project_description = "Vui lòng mô tả dự án.";
      } else if (form.project_description.trim().length < 20) {
        nextErrors.project_description = "Mô tả dự án cần ít nhất 20 ký tự.";
      }

      if (form.team_size && Number(form.team_size) <= 0) {
        nextErrors.team_size = "Quy mô nhóm phải lớn hơn 0.";
      }
    }
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...
      project_name: form.has_project ? form.project_name.trim() || null : null,

      project_field: form.has_project
        ? form.project_field.trim() || null
        : null,

      startup_stage: form.has_project ? form.startup_stage || null : null,

      project_description: form.has_project
        ? form.project_description.trim() || null
        : null,

      incubation_status: form.has_project
        ? form.incubation_status
        : "Chưa tham gia",

      female_founder:
        form.has_project && form.gender === "FEMALE"
          ? Boolean(form.female_founder)
          : false,

      team_size:
        form.has_project && form.team_size ? Number(form.team_size) : null,
      ...
      <div className="mx-auto max-w-3xl px-6 py-20">
        <div className="rounded-3xl border border-green-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            SIHUB sẽ gửi thông tin xác nhận đến email của bạn sau khi hồ sơ được
            duyệt.
          </p>
          ...
          <form onSubmit={submit} noValidate className="mt-8 space-y-8">
            <FormSection title="Thông tin cá nhân">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Field label="Đơn vị/Công ty" error={errors.company}>
                  <input
                    type="text"
                    value={form.company}
                    onChange={(event) =>
            ...
            <FormSection title="Thông tin dự án">
              <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <input
                  type="checkbox"
                  checked={form.has_project}
                  onChange={(event) => {
                    const checked = event.target.checked;

                    updateField("has_project", checked);
                ...
                <div>
                  <p className="font-semibold text-slate-800">
                    Tôi đang có dự án hoặc ý tưởng khởi nghiệp
                  </p>

                  <p className="mt-1 text-sm text-slate-500">
                    Chọn mục này để cung cấp thêm thông tin về dự án.
                  </p>
                </div>
              </label>
              ...
              {form.has_project && (
                <div className="mt-6 grid gap-5 md:grid-cols-2">
                  <Field label="Tên dự án" required error={errors.project_name}>
                    <input
                      type="text"
                      ...
                      maxLength={255}
                      placeholder="Tên dự án hoặc ý tưởng"
                  ...
                  <Field label="Quy mô nhóm" error={errors.team_size}>
                    <input
                      type="number"
                      min="1"
                      max="10000"
                      ...
                      className={inputClass(errors.team_size)}
                    />
                  </Field>

                  <Field label="Trạng thái ươm tạo">
                    <select
                      value={form.incubation_status}
                      onChange={(event) =>
                        updateField("incubation_status", event.target.value)
                      }
                  ...
                  {form.gender === "FEMALE" && (
                    <div className="flex items-end">
                      <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 px-4 py-3">
                        <input
                          type="checkbox"
                          checked={form.female_founder}
                          onChange={(event) =>
                            updateField("female_founder", event.target.checked)
                          }
                          className="h-4 w-4 accent-green-600"
                        />

                        <span className="text-sm font-medium text-slate-700">
                          Dự án có nữ sáng lập
                        </span>
                      </label>
                    </div>
                  )}
                  ...
                  <div className="md:col-span-2">
                    ...
                    >
                      <textarea
                        ...
                        maxLength={3000}
                        placeholder="Mô tả vấn đề, giải pháp, khách hàng mục tiêu và tiến độ hiện tại..."
            ...
            <FormSection title="Ghi chú">
              <Field label="Thông tin bổ sung" error={errors.note}>
                <textarea
                  rows={4}
                  value={form.note}
                  onChange={(event) => updateField("note", event.target.value)}
                  maxLength={1000}
                  placeholder="Nhu cầu hỗ trợ hoặc thông tin cần SIHUB lưu ý..."
